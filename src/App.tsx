...
function App() {
  ...
  const executionEnabled = appMode === 'devnet' && !walletBusy && spotFresh && Number.isFinite(quantity) && quantity >= minimumTradeSizeSol && quoteMode === 'maker' && hasExecutableQuote(selectedQuote, tradeSide, quantity) && Boolean(getSettlementProgramId())
  ...
  const placeTrade = async () => {
    ...
    try {
      ...
      const signature = await submitSolanaTransaction(instructions, sendTransaction, walletAddress)
      ...
      await syncOnchainPositions(owner.toBase58())

      setWalletBalance(await getSolBalance(owner.toBase58()))
      ...
      notify(error instanceof Error ? error.message : 'Order was not submitted')
...
function MarketTicket({ appMode, quoteMode, series, kind, side, setSide, spot, quantity, quantityText, setQuantityText, walletConnected, executionEnabled, onOpenTrade, onGuard }: { appMode: AppMode; quoteMode: 'maker' | 'indicative' | 'offline'; series?: OptionSeries; kind: OptionKind; side: TradeSide; setSide: (side: TradeSide) => void; spot: number; quantity: number; quantityText: string; setQuantityText: (value: string) => void; walletConnected: boolean; executionEnabled: boolean; onOpenTrade: () => void; onGuard: () => void }) {
